...
import MainPackage.CustomerFrame;
...
public class Customer extends javax.swing.JInternalFrame {
    ...
    public void displayData(){
        try{
            ...
            ResultSet rs = dbc.getData("SELECT customer_id, first_name, last_name, phone_number, city, state, postal_code FROM tbl_customer");
            cust_tbl.setModel(DbUtils.resultSetToTableModel(rs));
            DefaultTableModel model = (DefaultTableModel) cust_tbl.getModel();
            String[] columnIdentifiers = {"Customer ID", "First Name", "Last Name", "Contact No.", "City", "State", "Postal Code"};
    ...
    private void initComponents() {
        ...
        search_btnLayout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, search_btnLayout.createSequentialGroup()
    ...
    Image newImg = img.getScaledInstance(cust.custimg_display.getWidth(), cust.custimg_display.getHeight(), Image.SCALE_SMOOTH);
    ...
    private void edit_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_edit_btnMouseClicked
        ...
        if(rowIndex < 0){
            ...
            TableModel model = cust_tbl.getModel();
            
            CustomerFrame cust = new CustomerFrame();
            cust.custid_inp.setText(""+model.getValueAt(rowIndex, 0));
            String cid = model.getValueAt(rowIndex, 0).toString();
            try{
            DBconnector dbc = new DBconnector();
            ResultSet rs = dbc.getData("SELECT * FROM tbl_customer WHERE customer_id = '"+cid+"' ");
            if(rs.next()){
            cust.custfname_inp.setText(rs.getString("first_name"));
            cust.custlname_inp.setText(rs.getString("last_name"));
            cust.custemail_inp.setText(rs.getString("email"));
            cust.custphone_inp.setText(rs.getString("phone_number"));
            cust.custbarangay_inp.setText(rs.getString("barangay"));
            cust.custcity_inp.setText(rs.getString("city"));
            cust.custstate_inp.setText(rs.getString("state"));
            cust.custpostal_inp.setText(rs.getString("postal_code"));
            cust.custimg_display.setIcon(ResizeImage(null, rs.getBytes("customer_image")));
            
            cust.setVisible(true);
            cust.action = "Update";
            cust.action_lbl.setText("UPDATE");
            JFrame mainFrame = (JFrame) SwingUtilities.getWindowAncestor(this);
            mainFrame.dispose();
            }else{
    ...
    private void delete_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_delete_btnMouseClicked
        ...
        if(rowIndex < 0){
            ...
            if(a == JOptionPane.YES_OPTION){
                ...
                dbc.deleteData(customer_id, "tbl_customer", "customer_id");
                displayData();
            }
        }
    ...
    private void print_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_print_btnMouseClicked
        MessageFormat header = new MessageFormat("Cafe Management System");
        MessageFormat footer = new MessageFormat("Product Management System Output");
        try{
            cust_tbl.print(JTable.PrintMode.FIT_WIDTH, header, footer);          
